...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        device_gpu = 'cuda:3'
        net.to(device=device_gpu)# net.cuda()
        net.train()
        ...
        loss_fn = torch.nn.BCELoss(reduction='sum').to(device=device_gpu)
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (
                ...
                img_feat_iter = img_feat_iter.to(device=device_gpu)
                # ques_ix_iter = ques_ix_iter.cuda()
                ques_ix_iter = ques_ix_iter.to(device=device_gpu)
                #ans_iter = ans_iter.cuda()
                ans_iter = ans_iter.to(device=device_gpu)
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        if state_dict is None:
            val_ckpt_flag = True
            print('Loading ckpt {}'.format(path))
            state_dict = torch.load(path, map_location='cuda:3')['state_dict']
            print('Finish!')
